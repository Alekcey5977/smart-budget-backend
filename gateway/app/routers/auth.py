...
async def get_me(current_user: Dict[Any, Any] = Depends(get_current_user)):
    ...
    - Для Swagger: параметр `token` в query string
      - Пример: `/auth/me?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...`
    
    - Для Postman/REST клиентов: заголовок `Authorization`
      - Формат: `Bearer <token>`
...
async def update_me(
    ...
    - `first_name` (строка) - Новое имя пользователя
      - Минимальная длина: 2 символа
      - Максимальная длина: 50 символов
      - Не может быть пустым или содержать только пробелы
      - Пример: "Петр"
    
    - `last_name` (строка) - Новая фамилия пользователя
      - Минимальная длина: 2 символа
      - Максимальная длина: 50 символов
      - Не может быть пустым или содержать только пробелы
      - Пример: "Петров"
    
    - `patronymic` (строка) - Новое отчество пользователя
      - Минимальная длина: 2 символа
      - Максимальная длина: 50 символов
      - Не может быть пустым или содержать только пробелы
      - Пример: "Петрович"
    ...
    🔑 **Способы передачи токена:**
    - Для Swagger: параметр `token` в query string
    - Для Postman: заголовок `Authorization: Bearer <token>`
