...
Downstream (users-service) мокается через httpx.AsyncClient.
...
class TestAddBankAccount:
    ...
    async def test_upstream_5xx_raises_http_status_error(self, client):
        """Upstream 500 → HTTPStatusError → gateway пробрасывает статус."""
        with patch("app.routers.bank_accounts.httpx.AsyncClient") as MockClient:
            mock_http = AsyncMock()
            MockClient.return_value.__aenter__.return_value = mock_http
